...
#
# Additional option for toracat-friendly, one-off, kernel-lts building.
# Only build the base kernel-lts (--with baseonly):
%define with_baseonly     %{?_with_baseonly:        1} %{?!_with_baseonly:        0}

%global KVERREL %{pkg_version}-%{pkg_release}.%{_target_cpu}

# If requested, only build base kernel-lts package.
%if %{with_baseonly}
%define with_doc 0
%define with_perf 0
%define with_tools 0
...
#
# List the packages required for the kernel-lts build.
#
...
#
# This macro does requires, provides, conflicts, obsoletes for a kernel-lts package.
...
#
# This macro creates an empty kernel-lts-<subpackage>-devel-matched package that
...
Provides: installonlypkg(kernel-lts-module)\
...
#
# This macro creates a kernel-lts-<subpackage> and its -devel.
#	%%define variant_summary The Linux kernel-lts compiled for <configuration>
...
# Files for 'make scripts' to succeed with kernel-lts-devel.
...
# Files for 'make prepare' to succeed with kernel-lts-devel.
...
%ifarch x86_64
# Files required for 'make prepare' to succeed with kernel-lts-devel.
...
# Identify modules in the kernel-lts-modules-extras package
...
# Don't include anything going into kernel-lts-modules-extra in the file lists.
xargs rm -fr < mod-extra.list

# Find all the module files and filter them out into the core and modules lists.
# This actually removes anything going into kernel-lts-modules from the directory.
...
# for when the kernel-lts-devel package is installed.
ln -sf /usr/src/kernels/%{KVERREL} $RPM_BUILD_ROOT/lib/modules/%{KVERREL}/build

# Move the generated vmlinux.h file into the kernel-lts-devel directory structure.
...
# kernel-lts headers_install will remove any header files in /usr/include that
# it doesn't install itself.

%if %{with_headers}
# Install kernel-lts headers
...
#
# This macro defines a %%post script for a kernel-lts*-devel package.
...
#
# This macro defines a %%post script for a kernel-lts*-modules-extra package.
...
#
# This macro defines a %%post script for a kernel-lts*-modules package.
# It also defines a %%postun script that does the same thing.
...
# This macro defines a %%posttrans script for a kernel-lts package.
...
#
# This macro defines the %%files sections for a kernel-lts package
